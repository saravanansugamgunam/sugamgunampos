...
<div class="container-fluid">
	...
	<div class="contentheader">
			<i class="icon-table"></i> Purchase Entry
			</div>
...
<a  href="index.php"><button class="btn btn-default btn-small" style="float: left;"><i class="icon icon-circle-arrow-left icon-large"></i> Back</button></a>
			<?php 
			include('../connect.php');
				$result = $db->prepare("SELECT * FROM products   ORDER BY qty ");
				$result->execute();
...
<a rel="facebox" href="addproduct.php"><Button type="submit" class="btn btn-info" style="float:right; width:230px; height:35px;" /><i class="icon-plus-sign icon-large"></i> Add New Purchase</button></a><br><br>
<table class="hoverTable" id="resultTable" data-responsive="table" style="text-align: left; background:white;" bgcolor="#00FF00" >
	<thead>
		<tr >
	...
	<tbody bgcolor="#00FF00">
			...
			function formatMoney($number, $fractional=false) {
				...
				include('../connect.php');
				$result = $db->prepare("
SELECT product_id AS ProductID, b.productshortcode as ShortCode, b.productname AS ProductName ,b.category AS Category,a.supplier AS Supplier,DATE_FORMAT(a.purchasedate, '%d/%m/%Y') AS PurchaseDate, 
a.cost AS Cost,b.price AS Price,a.qty AS PurchaseQty, a.qty*a.cost as Total,expiry_date   FROM 
 products AS a JOIN productmaster AS b ON a.product_id =b.productid ORDER BY purchaseentrydate DESC");
				$result->execute();
				for($i=0; $row = $result->fetch(); $i++){
				$total=$row['Total'];
				$availableqty=$row['PurchaseQty'];
				if ($availableqty < 10) {
			...
			?></td> 
			<td><?php echo $row['PurchaseQty']; ?></td>
			<td>
			<?php
