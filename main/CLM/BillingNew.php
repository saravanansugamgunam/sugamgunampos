...
<div class="modal fade" id="modalPaymentCollection" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-md">
            ...
            <div class="modal-body">
                ...
                <div class="form-row">
                    <div class="form-group col-md-6">
                        ...
                        <select class="form-control" id="cmbPaymentMode">
      ...
      ?>
                    ...
                    <div class="form-group col-md-6">
                        <label>Amount</label>
                        <input type="number" class="form-control" id="txtPaymentAmount" placeholder="Enter amount" />
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="txtTotalReceived" readonly>
                </div>

                <div class="form-group">
                    <label>Balance:</label>
                    <input type="text" class="form-control" id="txtPaymentBalance" readonly>
                </div>

            </div>

            <div class="modal-footer">
                <button class="btn btn-success" data-dismiss="modal" onclick='SaveSaleMaster();'>Done</button>
            </div>
        </div>
    </div>
</div>
    ...
    <div id="page-container" class="fade page-sidebar-minified page-header-fixed">
        ...
        function SaveSaleItems() {
            ...
            if (Number(BillQty) > Number(Currentstock)) {
                ...
                var TotalAmount = document.getElementById("txtNetAmount").value;
        ...
        function SaveSaleMaster() {
            var Invoice = document.getElementById("txtInvoiceNo").value;
            var TotalSaleAmount = parseFloat(document.getElementById("txtNettAmount").value || 0);
            var ReceivedAmount = parseFloat(document.getElementById("txtTotalReceived").value || 0);
            var BalanceAmount = TotalSaleAmount - ReceivedAmount;
            var GroupID = <?php echo $GroupID; ?>;

            if (ReceivedAmount < TotalSaleAmount) {
                if (GroupID === 1) {
                    // Allow due for admin group after confirmation
                    swal({
                        title: "Partial Payment",
                        text: "Received amount is less than bill total. Do you want to raise bill with balance due?",
                        icon: "warning",
                        buttons: {
                            cancel: "No",
                            confirm: {
                    ...
                    swal("Insufficient Payment", "Bill cannot be saved unless full amount is collected.", "error");
                }
            } else if (ReceivedAmount > TotalSaleAmount) {
                // Ask for confirmation if excess amount collected
                swal({
                    title: "Excess Payment",
                    text: "Received amount is more than the bill value. Do you want to proceed?",
                    icon: "info",
                    buttons: {
                        cancel: "Cancel",
                        confirm: {
                ...
                // Exact amount received — safe to proceed
                SaveBillingDetails();
            }
        }
        ...
        function SaveSaleMaster1() {
            ...
            var BalanceAmount = document.getElementById("txtPaymentBalance").value;
            var PaitentCode = document.getElementById("txtPaitentCode").value;
        ...
        function SaveBillingDetails() {
            ...
            var OldBalance = document.getElementById("txtOldDue").value;
            var ReceivedAmount = document.getElementById("txtTotalReceived").value;
            var NewBalance = document.getElementById("txtBalance").value;
            var DoctorCode = document.getElementById("cmbDoctor").value;
            var TokenNo = document.getElementById("txtTokenNo").value;
            var GrossAmount = document.getElementById("txtGrossAmt").value;
        ...
        function calculateFromPercent() {
            let mrp = parseFloat(document.getElementById("txtMRP").value) || 0;
            let discPercent = parseFloat(document.getElementById("txtDiscPercent").value) || 0;

            let discAmount = (mrp * discPercent) / 100;
            let netAmount = mrp - discAmount;

            document.getElementById("txtDiscAmount").value = discAmount.toFixed(2);
            document.getElementById("txtNetAmount").value = netAmount.toFixed(2);
        }

        function calculateFromAmount() {
            let mrp = parseFloat(document.getElementById("txtMRP").value) || 0;
            let discAmount = parseFloat(document.getElementById("txtDiscAmount").value) || 0;

            let discPercent = (discAmount / mrp) * 100;
            let netAmount = mrp - discAmount;

            document.getElementById("txtDiscPercent").value = discPercent.toFixed(2);
            document.getElementById("txtNetAmount").value = netAmount.toFixed(2);
        }
        ...
        $(document).ready(function() {
            $('#txtMobileNo').select2({
                ...
                allowClear: true,
                ajax: {
                    url: 'Load/PatientSearch.php',
                    type: 'POST',
                    dataType: 'json',
                    delay: 250,
                    data: function(params) {
                        return {
                            searchTerm: params.term // user typed term
                        };
                    },
            ...
            $('#txtMobileNo').on('select2:select', function(e) {
                const selected = e.params.data;
                $('#txtMobileNoq').val(selected.id);
                LoadPaitentDetails();
            });
        });
        </script>
        ...
        function addPayment() {
            const modeSelect = document.getElementById("cmbPaymentMode");
            const paymentModeCode = modeSelect.value;
            const paymentModeName = modeSelect.options[modeSelect.selectedIndex].text;
            const paymentAmount = parseFloat(document.getElementById("txtPaymentAmount").value);

            if (!paymentModeCode || isNaN(paymentAmount) || paymentAmount <= 0) {
                swal("Invalid Entry", "Please select a valid payment mode and amount.", "warning");
                return;
            }
        ...
        function renderPayments() {
            ...
            payments.forEach((p, index) => {
                total += p.amount;
                html += `<tr>
              <td>${p.name}</td> <!-- display readable name -->
              <td>${p.amount.toFixed(2)}</td>
              <td><button class="btn btn-xs btn-danger" onclick="removePayment(${index})">X</button></td>
            </tr>`;
            });

            document.getElementById("paymentTable").innerHTML = html;
            document.getElementById("txtTotalReceived").value = total.toFixed(2);

            const netAmount = parseFloat(document.getElementById("txtNettAmount").value || 0);
            const balance = netAmount - total;
            document.getElementById("txtPaymentBalance").value = balance.toFixed(2);
        }
                    ...
                    <div id='ModalProject' class='modal-window'>
                        <div class="row">
                            ...
                            <table>
                                ...
                                <tr>
                                    <td> <select class="form-control" id='cmbPaymentModeee' name='cmbPaymentModeee'
                ...
                <div class="panel panel-inverse">
                    <div class="panel-body">
                        ...
                        <div class="row">
                            ...
                            <!-- Sale Date -->
                            <div class="col-md-2">
                                <label for="dtSaleDate">Sale Date</label>
                                <input type="date" class="form-control" id="dtSaleDate" name="dtSaleDate"
      ...
      ?>
                            ...
                            <div class="col-md-2">
                                <label>Doctor</label>
                                <select class="form-control" id="cmbDoctor" name="cmbDoctor"
      ...
      ?>
                            ...
                            <div class="col-md-3">
                                ...
                                <select class="form-control select2" id="txtMobileNo" name="txtMobileNo"></select>
                                <input type="hidden" id="txtMobileNoq" />
                                <input type="hidden" id="txtPaitentCode" />
                            </div>

                            <!-- Barcode -->
                            <div class="col-md-2" style='display:none'>
                                <label>Barcode</label>
                                <input type="text" class="form-control" id="txtPaitentBarcode"
                                    onblur="LoadPaitentDetails();ClearPaitentDropdown();" />
                            </div>

                            <!-- Add Patient Button -->
                            <div class="col-md-1">
                                <label>&nbsp;</label>
                                <a href="AddPaitent.php?MID=18" class="btn btn-success btn-block">+</a>
                            </div>

                            <!-- Old Due -->
                            <div class="col-md-1">
                                <label>Old Due</label>
                                <input type="text" class="form-control" id="txtOldDue" value="0" disabled />
                            </div>
                            ...
                            <div class="col-md-1">
                                ...
                                <input type="text" class="form-control"
                                    style="font-size: 20px; box-shadow: 2px 2px 3px blue; border-radius: 5px; text-align: center;"
                                    id="txtTokenNo" name="txtTokenNo" maxlength="4" disabled />
                            </div>
                        ...
                        <fieldset>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id='cmbProductCode' name='cmbProductCode'
                                            onchange="LoadProductDetails();LoadSpeacailTokenTiming()">
                                            <option selected></option>
                                ...
                                <input style=" background-color:white;" type="hidden" name="txtShortcode"
                                            ...
                                            <div class="col-md-2">
                                                <label>MRP</label>
                                                <input type="number" class="form-control" id="txtMRP" name="txtMRP"
                                                    step="0.01" readonly>
                                            </div>

                                            <div class="col-md-1">
                                                <label>Discount %</label>
                                                <input type="number" class="form-control" id="txtDiscPercent"
                                                    name="txtDiscPercent" step="0.01"
                                                    onchange="calculateFromPercent();">
                                            </div>

                                            <div class="col-md-1">
                                                <label>Discount ₹</label>
                                                <input type="number" class="form-control" id="txtDiscAmount"
                                                    name="txtDiscAmount" step="0.01" onchange="calculateFromAmount();">
                                            </div>

                                            <div class="col-md-2">
                                                <label>Net Amount</label>
                                                <input type="number" class="form-control" id="txtNetAmount"
                                                    name="txtNetAmount" readonly>
                                            </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <button class="btn btn-info" data-toggle="modal"
                                                data-target="#modalPaymentCollection">
                                                Collect Payment
                                            </button>


                                            <a hidden href='#ModalProject'> <input type="button" class="btn btn-sm btn-success"
