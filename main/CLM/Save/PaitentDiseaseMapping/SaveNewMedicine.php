...
if(isset($_POST["NewMedicine"]))
{
 ...
 $NewMedicine = mysqli_real_escape_string($connection, $_POST["NewMedicine"]);     
 ...
 $UniqueID = mysqli_real_escape_string($connection, $_POST["UniqueID"]);
  ...
  try {
   if($PaitentID<>'')
   { 
      $QryAddDiagnosis = "insert into medicinehistorymaster   (medicinename) values
      ('$NewMedicine') "; 
   }
   ...
   if (mysqli_query($connection, $QryAddDiagnosis)) { 
      ...
      ('$PaitentID','$last_idBatch','Medicine History','$userid','$currentdate','$ClientID','$UniqueID','$Period') ";
