...
//insert.php
if(isset($_POST["NewHabbit"]))
{
  
 // echo "1";
 include("../../../../connect.php"); 
  $currentdate =date("Y-m-d"); 	 
 $NewHabbit = mysqli_real_escape_string($connection, $_POST["NewHabbit"]);     
 ...
 $UniqueID = mysqli_real_escape_string($connection, $_POST["UniqueID"]);
  ...
  try {
   if($PaitentID<>'')
   { 
      $QryAddDiagnosis = "insert into personalhabitmaster   (habbitname) values
      ('$NewHabbit') "; 
   }
   ...
   if (mysqli_query($connection, $QryAddDiagnosis)) { 
      ...
      ('$PaitentID','$last_idBatch','Habbit History','$userid','$currentdate','$ClientID','$UniqueID','$Period') ";
