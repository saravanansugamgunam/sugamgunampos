...
//insert.php
if(isset($_POST["NewDisease"]))
{
  
 // echo "1";
 include("../../../../connect.php"); 
  $currentdate =date("Y-m-d"); 	 
 $NewDisease = mysqli_real_escape_string($connection, $_POST["NewDisease"]);     
 ...
 $UniqueID = mysqli_real_escape_string($connection, $_POST["UniqueID"]);
  ...
  try {
   if($PaitentID<>'')
   { 
      $QryAddDiagnosis = "insert into diseasemaster (disease) values
      ('$NewDisease') "; 
   }
   ...
   if (mysqli_query($connection, $QryAddDiagnosis)) { 
      ...
      ('$PaitentID','$last_idBatch','Disease','$userid','$currentdate','$ClientID','$UniqueID') ";
