...
if(isset($_POST["PaitentID"]))
{
 ...
 $UniqueID = mysqli_real_escape_string($connection, $_POST["UniqueID"]);     
  ...
  try {
   ...
   { 
      $QryAddDiagnosis='';
      // $QryAddDiagnosis.= "delete from diseasemapping_paitent WHERE suggested='1';";
      ...
      WHERE conceptname<>'Symptoms' AND diseaseid IN (
      SELECT diseaseid FROM  diseasemapping WHERE conceptname='Symptoms' AND  conceptid IN(
      SELECT conceptid FROM  `diseasemapping_paitent`  WHERE paitientid='$PaitentID' AND clientid='$ClientID' 
      ...
      // diseasemapping WHERE conceptname='Symptoms' AND  conceptid IN(
      // SELECT conceptid FROM  `diseasemapping_paitent`  WHERE paitientid='$PaitentID' AND clientid='$ClientID' 
      // AND consultingdate='$currentdate') GROUP BY diseaseid
      // ON DUPLICATE KEY UPDATE diseasemapping_paitent.mappingid = diseasemapping_paitent.mappingid; "; 
