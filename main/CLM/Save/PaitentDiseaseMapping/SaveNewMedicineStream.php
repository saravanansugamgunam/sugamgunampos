...
//insert.php
if(isset($_POST["NewMedicineStream"]))
{
  
 // echo "1";
 include("../../../../connect.php"); 
  $currentdate =date("Y-m-d"); 	 
 $NewMedicineStream = mysqli_real_escape_string($connection, $_POST["NewMedicineStream"]);     
 ...
 $UniqueID = mysqli_real_escape_string($connection, $_POST["UniqueID"]);
  ...
  try {
   if($PaitentID<>'')
   { 
      $QryAddDiagnosis = "insert into previousmedicationmaster (medicationname) values
      ('$NewMedicineStream') "; 
   }
   ...
   if (mysqli_query($connection, $QryAddDiagnosis)) { 
      ...
      ('$PaitentID','$last_idBatch','Medical Stream History','$userid','$currentdate','$ClientID','$UniqueID','$Period') ";
