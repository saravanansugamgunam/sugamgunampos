...
//insert.php
if(isset($_POST["NewPathology"]))
{
  
 // echo "1";
 include("../../../../connect.php"); 
  $currentdate =date("Y-m-d"); 	 
 $NewPathology = mysqli_real_escape_string($connection, $_POST["NewPathology"]);     
 ...
 $UniqueID = mysqli_real_escape_string($connection, $_POST["UniqueID"]);    
  ...
  try {
   if($PaitentID<>'')
   { 
      $QryAddDiagnosis = "insert into pathologymaster (pathology) values
      ('$NewPathology') "; 
   }
   ...
   if (mysqli_query($connection, $QryAddDiagnosis)) { 
      ...
      ('$PaitentID','$last_idBatch','Pathology','$userid','$currentdate','$ClientID','$UniqueID') ";
