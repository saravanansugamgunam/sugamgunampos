 ...
 $UniqueID = mysqli_real_escape_string($connection, $_POST["UniqueID"]);     
  ...
  try {
   ...
   { 
      ...
      or  conceptname ='Acupoints'  or conceptname ='Therapy' or conceptname ='Medicine';";
      ...
      WHERE conceptname NOT IN ('Symptoms','Diagnosis','Disease')   AND diseaseid IN (
      SELECT diseaseid FROM  diseasemapping WHERE conceptname='Diagnosis' AND  conceptid IN(
      SELECT conceptid FROM  `diseasemapping_paitent`  WHERE  conceptname='Diagnosis' AND 
      ...
      // $QryAddDiagnosis.=" insert into diseasemapping_paitent (paitientid,conceptid,conceptname,addedby,consultingdate,
      // clientid,suggested,diseasemappinguniqueid)  SELECT '$PaitentID',diseaseid,'Disease',
      // '$userid','$currentdate','$ClientID','1','$UniqueID' FROM 
      // diseasemapping WHERE conceptname='Diagnosis' AND  conceptid IN(
      // SELECT conceptid FROM  `diseasemapping_paitent`  WHERE   conceptname='Diagnosis' AND paitientid='$PaitentID' AND clientid='$ClientID' 
      // AND consultingdate='$currentdate') GROUP BY diseaseid
      // ON DUPLICATE KEY UPDATE diseasemapping_paitent.mappingid = diseasemapping_paitent.mappingid; "; 
