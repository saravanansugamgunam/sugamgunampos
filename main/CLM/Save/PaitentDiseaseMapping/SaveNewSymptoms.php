...
//insert.php
if(isset($_POST["NewSymptoms"]))
{
  
 // echo "1";
 include("../../../../connect.php"); 
  $currentdate =date("Y-m-d"); 	 
 $NewSymptoms = mysqli_real_escape_string($connection, $_POST["NewSymptoms"]);     
 ...
 $UniqueID = mysqli_real_escape_string($connection, $_POST["UniqueID"]);       
  ...
  try {
   if($PaitentID<>'')
   { 
      $QryAddDiagnosis = "insert into symptomsmaster (symptoms) values
      ('$NewSymptoms') "; 
   }
   ...
   if (mysqli_query($connection, $QryAddDiagnosis)) { 
      ...
      ('$PaitentID','$last_idBatch','Symptoms','$userid','$currentdate','$ClientID',
      '$UniqueID') ";
