...
// b.mobileno,b.paitentname,c.mobileno,ROUND(b.topay-b.receipt,0) AS OldDue, 
...
c.mobileno,d.mobileno, c.topay-c.receipt
...
WHERE a.therapystatus in('Booked','WIP') and  $PaitentID and therapystatus in('Booked','WIP') 
and  a.revisedtherapydate between '$FromDate' and '$ToDate'
...
c.mobileno,d.mobileno,CONCAT(e.consultationname,' (Sittings-',a.totalsitting,')'),a.fees, c.topay-c.receipt 
...
JOIN usermaster AS d ON a.doctorid=d.userid
JOIN consultationmaster AS e ON a.therapyid=e.consultationid
WHERE a.therapystatus in('Closed','Booked','Cancelled','WIP') and  $PaitentID  and 
therapystatus in('Closed','Booked','Cancelled','WIP') 
and  a.revisedtherapydate between '$FromDate' and '$ToDate'
...
if($ID=='99999')
{
   ...
   <td   id='DueAmount' width='%'>$data[9]</td> 
