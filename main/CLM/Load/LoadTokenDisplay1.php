...
AND tokenstatus ='Open' AND doctorid ='1' and a.createdon < 140001 and locationcode='$LocationCode'   ORDER BY revisedtokennumber LIMIT 5 OFFSET 1");
}
   else
  {
	  $result = mysqli_query($connection, "SELECT tokennumber,paitentname,
(SELECT paitentnamedisplay FROM tokensettings where clientid ='$LocationCode') AS DisplayName FROM  tokenmaster AS a JOIN paitentmaster AS b 
ON a.`paitentcode`=b.`paitentid` WHERE a.`date` =CURRENT_DATE 
AND tokenstatus ='Open' AND doctorid ='1'  AND createdon > 140000  and locationcode='$LocationCode' 
  ORDER BY revisedtokennumber LIMIT 5 OFFSET 1");
  }
