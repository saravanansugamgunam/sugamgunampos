...
CASE c.consultationid
	WHEN 6 THEN 'New'
        WHEN 17 THEN 'New'         
        ELSE 'Followup'
	END customerType ,paitentcode, invoicenumber,tokenstatus,tokenid,onlinetokenflag, a.createdon,a.doctorchangeflag
...
WHERE DATE = CURRENT_DATE and locationcode like('$Location') and a.createdon > 140000 and tokenstatus
   IN ('$TokenStatus') and a.doctorid ='$DoctorCode' order by tokennumber ");
	...
	while ($data = mysqli_fetch_row($result)) {
		...
		if ($data[10] == 1) {
			if ($data[9] == '150000' || $data[9] == '100000') {
				echo "<div class='widget widget-stats bg-orange'>";
			} else {
				echo "<div class='widget widget-stats bg-orange'>";
			}
		} else {
		...
		if ($data[6] == 'Open') {
			echo "
							...
							onclick='GetTokenNo($data[7],$data[5]);'>
        ...
        <i style='float:right;color:#c1da48' onclick='SaveConsultingClosure($data[4],$data[5],$data[0]);' class='fa fa-2x fa-check-circle-o'></i></a>
		
		
		<a style='float:right;color:yellow'   href='#'  >
        <i style='float:right;color:yellow' onclick='PlaySound($data[0]);' class='fa fa-2x fa-bullhorn'></i></a>
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ";
		} else {
		}
		...
		if ($data[6] == 'Open') {
			...
			echo "<div class='stats-link'> 
			 <a href='ConsultingView.php?PID=$data[4]&INV=$data[5]&TID=$data[0]&S=O&MID=31' > ";
			if ($data[8] == '1') {
			...
			echo "<div class='stats-link'> 
							<a href='ConsultingView.php?PID=$data[4]&INV=$data[5]&TID=$data[0]&S=C&MID=31' ><i class='fa fa-check-circle-o'></i> View Details (Completed) </a>
						</div>";
		} else if ($data[6] == 'Cancelled') {
			echo "<div class='stats-link'> 
							<a href='ConsultingView.php?PID=$data[4]&INV=$data[5]&TID=$data[0]&S=X&MID=31' ><i class='fa fa-times-circle-o'></i> View Details (Cancelled) </a>
						</div>";
		}
