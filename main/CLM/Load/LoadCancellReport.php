...
// $result = mysqli_query($connection, " 
 ...
 $result = mysqli_query($connection, " 
 SELECT DATE_FORMAT(a.billdate,'%d-%m-%Y') ,a.consultationuniquebill,e.`locationname`, a.billid,
 CONCAT(b.paitentname,'(',b.mobileno,')') AS paitent,c.username,a.totalamount,
 f.`remarks`,g.`username` ,cancelledstatus   
...
consultingdetails AS d ON a.consultationuniquebill = d.consultationuniquebill
 JOIN locationmaster AS e ON a.locationcode=e.`locationcode`
 LEFT JOIN billcancelldetails AS f ON a.consultationuniquebill=f.cancelledbillno
 LEFT JOIN `usermaster` AS g ON f.`cancelledby`=g.`userid`
...
while($data = mysqli_fetch_row($result))
{
  ...
  <td >$data[2]</td>  
   ...
   <td width='%'>$data[4]</td>   
   <td width='%'>$data[5]</td>          
   <td width='%'style='text-align:right;' >"; echo formatMoney($data[6], true); 
   
   echo "<td width='%'>$data[7]</td> ";
   echo "<td width='%'>$data[8]</td> ";
...
// $result = mysqli_query($connection, " 
 ...
 WHERE a.saledate BETWEEN '$ActualFromDate' AND '$ActualToDate' and a.locationcode ='$LocationCode'   AND a.transactiontype NOT IN('Outstanding','Cancelled') AND cancellstatus =1 and 
...
$result = mysqli_query($connection, "  
 ...
 LEFT JOIN courierdetails AS d ON  a.saleuniqueno=d.invoicenumber
 WHERE a.saledate BETWEEN '$ActualFromDate' AND '$ActualToDate'  and locationcode ='$LocationCode'  and a.transactiontype not in('Outstanding') and cancellstatus =0  and 
 saletype like ('%".$BillMode."%') 
