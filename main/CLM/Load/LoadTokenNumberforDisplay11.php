...
AND  DATE =CURRENT_DATE() AND doctorid ='1'  AND createdon < 140001 
ORDER BY revisedtokennumber) AND  
tokenstatus = 'Open'
AND  DATE =CURRENT_DATE() AND doctorid ='1'  
) LIMIT 1  " ); 
  }
   else
  {
...
SELECT tokenid FROM tokenmaster WHERE revisedtokennumber IN( 
SELECT MIN(revisedtokennumber)+1 FROM tokenmaster WHERE tokenstatus = 'Open'
AND  DATE =CURRENT_DATE() AND doctorid ='1'  AND createdon > 140000 
ORDER BY revisedtokennumber) AND  
tokenstatus = 'Open'
AND  DATE =CURRENT_DATE() AND doctorid ='1'  
) LIMIT 1 " ); 
