...
function formatMoney($number, $fractional=false) {
 ...
 if($PaymentMode=='No')
   ...
   SELECT t.transactiontype,t.Income,t.Expense,
...
WHERE  a.`date` BETWEEN '$ActualFromDate' AND '$ActualToDate' and a.clientid like ('$Location')  AND transactionstatus ='Live' 
 and transactiongroup ='Clinic' and transactiontype in ('Therapy Payment','IncomeEntry','DoctorFee') and transactionstatus='Live' 
...
WHERE  a.`date` BETWEEN  '$ActualFromDate' AND '$ActualToDate'  and a.clientid like ('$Location')   
 ...
 and b.paymentmodecode like ('$SelectedPaymentType')
 GROUP BY a.`transactiontype` ) AS a
 GROUP BY a.transactiontype ) t
...
$result = mysqli_query($connection, " 
   ...
   SELECT t.transactiontype,t.paymentmode,t.Income,t.Expense,
...
ON a.`paymentmode`=b.`paymentmodecode`
WHERE  a.`date` BETWEEN '$ActualFromDate' AND '$ActualToDate' and a.clientid like ('$Location')  AND transactionstatus ='Live'  and transactiongroup ='Clinic' 
and transactiontype in ('Therapy Payment','IncomeEntry','DoctorFee') and transactionstatus='Live' 
 and b.paymentmodecode like ('$SelectedPaymentType')
...
WHERE  a.`date` BETWEEN  '$ActualFromDate' AND '$ActualToDate'  and a.clientid like ('$Location')   
  and transactiongroup ='Clinic' and  transactiontype in ('ExpenseEntry','Salary','Supplier Payment','Therapy Share') and transactionstatus='Live' 
 and b.paymentmodecode like ('$SelectedPaymentType')
 GROUP BY a.`transactiontype`,b.paymentmode ) AS a
 GROUP BY a.transactiontype,a.paymentmode ) t
