...
if(isset($_POST["Location"]))
{
 ...
 include("../../../connect.php"); 
  ...
  {
	  $result = mysqli_query($connection, "SELECT a.tokennumber,b.paitentname,
	  d.consultationname,
	  CASE c.consultationid
		  WHEN 6 THEN 'New'
			  WHEN 17 THEN 'New'         
			  ELSE 'Followup'
		  END customerType ,paitentcode, invoicenumber,a.tokenstatus,tokenid,refundstatus
	  ...
	  JOIN consultingbillmaster AS e ON a.`invoicenumber`=e.`consultationuniquebill`
WHERE DATE = CURRENT_DATE and a.locationcode like('$Location')  and a.createdon < 140001  
and a.tokenstatus  IN ('$TokenStatus') and a.doctorid ='$userid' order by a.tokennumber ");
  }
  else
  {
	   $result = mysqli_query($connection, "SELECT a.tokennumber,b.paitentname,
	   ...
	   JOIN `consultationmaster` AS d ON c.consultationid=d.consultationid
	   JOIN consultingbillmaster AS e ON a.`invoicenumber`=e.`consultationuniquebill`
WHERE DATE = CURRENT_DATE and a.locationcode like('$Location') and a.createdon > 140000 and a.tokenstatus
   IN ('$TokenStatus') and a.doctorid ='$userid' order by a.tokennumber ");
  }
 ...
 while($data = mysqli_fetch_row($result))
{
 ...
 echo "<div class='col-md-3 col-sm-6'>";
					...
					if( $data[8] == 'FullRefund')
					{
					echo "<div class='widget widget-stats bg-red'>";	
					}
					else if( $data[8] == 'PartialRefund')
					{
