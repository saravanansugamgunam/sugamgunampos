...
<body onload="Reset();">
    ...
    <div id="modalConfirmPatientINtime" class="modal fade" role="dialog">
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h4 class="modal-title">Patient In Time Confirmation</h4>
                </div>

                <div class="modal-body">
                    
                    <label>Arival Status</label>
                    <br>
                    <select class='form-control' id='cmbArivalStatus' name='cmbArivalStatus'>
                    ...
                    <label>Patient Arival Time</label>
                    <br>
...
?>
                    ...
                    <input type='datetime-local' id='dtArivalTime' name='dtArivalTime' class='form-control'
                        value='<?php echo date('Y-m-d H:i'); ?>' />
                    <input type='hidden' id='txtPatientName' name='txtPatientName' />
                    <input type='hidden' id='txtPatientMobile' name='txtPatientMobile' />
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-info" onclick="UpdatePatientArival();"
                        data-dismiss="modal">Update</button>
                    <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                </div>
            </div>
    ...
    <div id="modalConfirmPatientINtime_therapy" class="modal fade" role="dialog">
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h4 class="modal-title">Patient In Time Confirmation - Therapy</h4>
                </div>
                ...
                <div class="modal-body">
                    ...
                    <label>Arival Status</label>
                    <br>
                    <select class='form-control' id='cmbArivalStatusTherapy' name='cmbArivalStatusTherapy'>
                        <option value='1'>Arrived</option>
                        <option value='0'>Not Arrived</option>
                    </select>
...
?>
                    ...
                    <input type='datetime-local' id='dtArivalTimeTherapy' name='dtArivalTimeTherapy' class='form-control'
                        value='<?php echo date('Y-m-d H:i'); ?>' />
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-info" onclick="UpdatePatientArivalTherpay();"
                        data-dismiss="modal">Update</button>
                    <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                </div>
            </div>
    ...
    function UpdatePatientArival() {
        var TokenID = document.getElementById("txtTokenNo").value;
       var ArivalStatus = document.getElementById("cmbArivalStatus").value;
        var datas = "&TokenID=" + encodeURIComponent(TokenID) + "&ArivalStatus=" + encodeURIComponent(ArivalStatus);

        $.ajax({
            url: "Save/UpdatePatientArival.php",
            method: "POST",
            data: datas,
            success: function(data) {
                //   alert(data);
                ...
                if (data == 1) {
                    Reset();
                    swal("Patient Status Updated !", "Sucessfully", "success");

                } else {
                    Reset();
                    swal("Alert!", 'Error Updating Patient Status', "warning");
    ...
    function UpdatePatientArivalTherpay() {
        var TokenID = document.getElementById("txtTokenNo").value;
       var ArivalStatus = document.getElementById("cmbArivalStatusTherapy").value;
        var datas = "&TokenID=" + encodeURIComponent(TokenID) + "&ArivalStatus=" + encodeURIComponent(ArivalStatus);

        $.ajax({
            url: "Save/UpdatePatientArivalTherapy.php",
            method: "POST",
            data: datas,
            success: function(data) {
                // alert(data);
...
?>
        ...
        function LoadTokenDetails() {
            ...
            var Type = $("input[name='rdConsultingType']:checked").val();
            ...
            var datas = "&Location=" + Location + "&TokenStatus=" +
                TokenStatus + "&DoctorCode=" + DoctorCode + "&Session=" + Session +
                "&Type=" + Type;

            $.ajax({
                url: "Load/LoadTokenDetailsNew1.php",
                method: "POST",
                data: datas,
                success: function(data) {
        ...
        function SaveConsultingClosure(x, y, z, a) {
                ...
                <div class="col-md-12">
                    ...
                    <div class="panel panel-inverse" data-sortable-id="form-stuff-1">
                        ...
                        <div class="panel-body">
                            ...
                            <select style='border-radius: 4px;display:none; padding: 5px;' id='cmbLocation'
                                name='cmbLocation'>
                            ...
                            <label class="radio-inline">
                                <input type="radio" name="rdConsultingType" value="Consultation" checked
                                    onclick="LoadTokenDetails();" />
                                Consulting
                            </label>
                            <label class="radio-inline">
                                <input type="radio" name="rdConsultingType" value="ReConsultation"
                                    onclick="LoadTokenDetails();" />
                                    Re-Consulting
                            </label>
                            <label class="radio-inline">
                                <input type="radio" name="rdConsultingType" value="Diagnosis"
                                    onclick="LoadTokenDetails();" />
                                Diagnosis
                            </label>
                            ...
                            <select style='border-radius: 4px; padding: 5px; ' hidden id='cmbType' name='cmbType'
                                onchange='LoadTokenDetails();'>
                                <option value='Consultation'>Consultation</option>
                                <option value='ReConsultation'>Re-Consultation</option>
                            </select>

                            &nbsp;&nbsp;&nbsp;


                            <select style='border-radius: 4px; padding: 5px;' id='cmbSession' name='cmbSession'
                                onchange='LoadTokenDetails();'>

                                <?php
                                $CurrentTime = date("His");
                                if($CurrentTime < 140001)
                                { $SelectMorningSession ='selected';}
                                else{$SelectEveningSession ='selected';}

    echo "<option value ='Morning' $SelectMorningSession  >Morning</option>";
    echo "<option value ='Evening' $SelectEveningSession >Evening</option>";
    ...
    $sqli = "SELECT userid,username FROM usermaster where designationid in('8','9') and
  activestatus='Active' and userid not in('$UserID') and  username not in ('Counter') order by username";
    $result = mysqli_query($connection, $sqli);
    while ($row = mysqli_fetch_array($result)) {
        # code...
