<?php
/* ==== Safe bootstrap: paste at the very top of ConsultingView.php ==== */
include("../../connect.php");
...
/* Security headers (sent at shutdown to avoid "headers already sent") */
if (!function_exists('_send_security_headers_once')) {
  function _send_security_headers_once() {
    static $sent = false; if ($sent) return; $sent = true;
    if (!headers_sent()) {
      header('X-Content-Type-Options: nosniff');
      header('Referrer-Policy: no-referrer-when-downgrade');
      header('Permissions-Policy: camera=(), microphone=(), geolocation=()');
      // header('X-Frame-Options: DENY'); // optional
      // header("Content-Security-Policy: default-src 'self' 'unsafe-inline' 'unsafe-eval' data: https:;"); // optional
    }
  }
  register_shutdown_function('_send_security_headers_once');
}

/* Helpers (HTML escape, 2-decimals, date conversions) */
if (!function_exists('e'))  { function e($v){ return htmlspecialchars((string)($v ?? ''), ENT_QUOTES, 'UTF-8'); } }
if (!function_exists('f2')) { function f2($n){ return number_format((float)$n, 2, '.', ''); } }
if (!function_exists('dmy')){ function dmy($iso){ return $iso ? date('d/m/Y', strtotime($iso)) : ''; } }
if (!function_exists('ymd')){ function ymd($dmy){ if(!$dmy) return null; $p=preg_split('~/~',$dmy); if(count($p)!=3) return null; [$d,$m,$y]=$p; return sprintf('%04d-%02d-%02d',(int)$y,(int)$m,(int)$d); } }

/* CSRF token */
if (empty($_SESSION['csrf_token'])) { $_SESSION['csrf_token'] = bin2hex(random_bytes(32)); }

/* POST guard (call at start of any POST handler) */
if (!function_exists('require_valid_csrf')) {
  function require_valid_csrf() {
    $ok = isset($_POST['csrf_token'], $_SESSION['csrf_token'])
          && hash_equals($_SESSION['csrf_token'], $_POST['csrf_token']);
    if (!$ok) { http_response_code(400); exit('Invalid or missing CSRF token. Please refresh and try again.'); }
  }
}

/* Auto-inject CSRF hidden input into every <form ...> tag via output buffering */
if (!headers_sent()) {
  ob_start(function($html) {
    if (!isset($_SESSION['csrf_token'])) return $html;
    $token = e($_SESSION['csrf_token']);
    // Insert hidden input right after each <form ...>
    return preg_replace('~(<form\\b[^>]*>)~i', "\\1\n  <input type=\"hidden\" name=\"csrf_token\" value=\"{$token}\">", $html);
  });
}
/* ==== end bootstrap ==== */
?>
...
?>
        ...
        <!-- begin #content -->
        <script>
        function LoadConsultingHistory() {
            var Dumy = 0;
            var PaitentID = document.getElementById("txtPaitentID").value;
        ...
        function LoadPaitentTransaction() {
            ...
            // alert(datas);
            $.ajax({
                url: "LoadPaymentTransaction.php",
                method: "POST",
                data: datas,
                success: function(data) {
                    // alert(data);
        ...
        <div id="myModalRefund" class="modal fade" role="dialog">
            <div class="modal-dialog">
                ...
                function LoadCaseHistoryCurrent() {
                    ...
                    var datas = "&InvoiceNo=" + InvoiceNo;
                    $.ajax({
                        url: "LoadCaseHistoryCurrent.php",
                        method: "POST",
                        data: datas,
                        dataType: "json",
                        success: function(data) {
                        ...
                        <div class="panel-body">
                            ...
                            <div class="col-md-12">
                                ...
                                <ul class="nav nav-tabs">
                                    <li class=" " onclick='LoadCaseHistoryCurrent();'><a
                                    ...
                                    <li class="" onclick='LoadConsultingHistory();'><a href="#default-tab-3"
                                    ...
                                    <li class="" onclick='LoadDocumentHistory();'><a href="#default-tab-2"
