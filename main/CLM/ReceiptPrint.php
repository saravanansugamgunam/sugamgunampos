...
<body onload='PrintOnLoad()'>
    ...
    <link href="https://fonts.googleapis.com/css?family=Open+Sans:300,400,600,700" rel="styleshe    et">
   ...
   (SELECT SUM(newbalance) FROM consultingbillmaster WHERE consultationuniquebill  ='$Invoice' ) AS CurrentBalance,
    ...
    (SELECT timing FROM consultingtiming  WHERE id IN (
        SELECT timeid FROM  consultingbillmaster WHERE consultationuniquebill ='$Invoice' ))   AS Timing,
          (SELECT einvoiceno FROM consultingbillmaster WHERE consultationuniquebill  ='$Invoice' ) AS eInvoice
	");
...
union 
    ...
    <button onclick='SendSMS()' class="hidden-print">SMS</button>
    <button onclick='SendEInvoice()' class="hidden-print">Send E-Invoice(Whatsapp)</button>
     
                    


    <button type="button" class="hidden-print" data-toggle="modal" data-target="#myModal">SMS to Alternate Mobile
    </button>
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Send Message - Alternate Number</h4>
                </div>
                <div class="modal-body">
                    <label>
                        New Mobile No
                    </label>
                    <input class='form-control' type='number' id='txtNewMobileNo' name='txtNewMobileNo'
                        value='<?php echo $MobileNo; ?>' />
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal" onclick='SendSMS()'>Send
                        Message</button>
                </div>
            </div>
...
function SendEInvoice() { 
            ...
            var PaitentName = "<?php echo $PaitentName; ?>";
    ...
    function SendSMS() {
        ...
        var TotalSaleAmount = '<?php echo $NettAmount; ?>';

        var MobileNo = document.getElementById("txtNewMobileNo").value;



        var PaitentName = '<?php echo $PaitentName; ?>';

        var TokenNo = '<?php echo $Token; ?>';


        var datas = "&TotalSaleAmount=" + TotalSaleAmount +
            "&MobileNo=" + MobileNo +
            "&TokenNo=" + TokenNo +
            "&PaitentName=" + PaitentName;
