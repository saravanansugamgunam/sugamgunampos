...
ob_start();
   ...
   (SELECT  paitentname FROM salemaster_estimate AS a JOIN paitentmaster AS b ON a.paitientcode = b.paitentid WHERE saleuniqueno ='$Invoice') AS PaitentName,
   (SELECT  mobileno FROM salemaster_estimate AS a JOIN paitentmaster AS b ON a.paitientcode = b.paitentid WHERE saleuniqueno ='$Invoice') AS MobileNo,
   
   (SELECT CONCAT('EST','-',saleid) FROM salemaster_estimate WHERE saleuniqueno  ='$Invoice' ) AS BillNo,
   (SELECT DATE_FORMAT(saledate,'%d-%m-%Y') FROM salemaster_estimate WHERE saleuniqueno  ='$Invoice' ) AS BillDate,
   
   (SELECT SUM(saleqty*mrp) FROM newsaleitems WHERE invoiceno  ='$Invoice' ) AS Total,
   (SELECT SUM(discountamount) FROM salemaster_estimate WHERE saleuniqueno  ='$Invoice' ) AS Discount, 
   (SELECT SUM(nettamount) FROM salemaster_estimate WHERE saleuniqueno  ='$Invoice' ) AS NettAmount, 
   (SELECT SUM(oldbalance) FROM salemaster_estimate WHERE saleuniqueno  ='$Invoice' ) AS PreviousBalance, 
   (SELECT SUM(amount) FROM salepaymentdetails WHERE invoiceno  ='$Invoice' )  AS Paid,
   (SELECT SUM(newbalance) FROM salemaster_estimate WHERE saleuniqueno  ='$Invoice' ) AS CurrentBalance,
   (SELECT SUM(couriercharges) FROM salemaster_estimate WHERE saleuniqueno  ='$Invoice' ) AS CourierCharge,
   (SELECT invoiceno FROM salemaster_estimate WHERE saleuniqueno  ='$Invoice' limit 1 ) AS LocationCode,
   (SELECT billtype FROM salemaster_estimate WHERE saleuniqueno  ='$Invoice' limit 1 ) AS BillType,
   (SELECT deliverystatus FROM salemaster_estimate WHERE saleuniqueno  ='$Invoice' limit 1 ) AS DeliveryStatus,
   (SELECT saleqty FROM salemaster_estimate WHERE saleuniqueno  ='$Invoice' limit 1 ) AS TotalQty,
   (SELECT transactiontype FROM salemaster_estimate WHERE saleuniqueno  ='$Invoice' limit 1 ) AS TransactionType
...
?>
         <div class="ticket">
             <p class="centered" style="display:nonde;"><u><b>Estimate <?php 
			if($BillLocationCode=='EST')
			{
				echo "VAN";
			}
			else if($BillLocationCode=='EST')
			{
				echo "CHE";
			}
			else if($BillLocationCode=='EST')
             ...
             <center>
				...
				if($BillType=='Regular')
				{
                    if($DeliveryStatus==1)
                    {
                        ...
                        SELECT   shortcode,sum(saleqty), nettamount  FROM newsaleitems
          WHERE invoiceno  ='$Invoice' group by shortcode, nettamount  "); 
                    }
                    else
                    {
                        ...
                        {
                            $result = mysqli_query($connection, "   
                            SELECT   shortcode,sum(saleqty), nettamount  FROM newsaleitemsproduct
                            WHERE invoiceno  ='$Invoice' group by shortcode, nettamount "); 
                        }
				...
				{
						$result = mysqli_query($connection, "   
				 SELECT   upper(productname),saleqty, nettamount  FROM newsaleitems
   WHERE invoiceno  ='$Invoice'; "); 
           ...
           <br>
             ...
             <label style='border-left: 1px solid white;  font-size:10px'> 
          
         
                <img  width="80" height="80"
                     src="../assets/img/SugamgunamHerbalsQR.png"  
