...
// $result = mysqli_query($connection, " 
   // SELECT d.paitentid,'-',d.paitentname,d.mobileno,'-' ,
// ROUND((d.topay-receipt),0) AS balance,d.email FROM  
  // paitentmaster AS d WHERE   (topay-receipt) > 1
  
  // ");
  
  
$result = mysqli_query($connection, " 
  SELECT d.paitentid,'-',d.paitentname,d.mobileno,DATE_FORMAT(f.TransactionDate,'%d-%m-%Y') ,
ROUND((d.topay-receipt),0) AS balance,d.email FROM  
  paitentmaster AS d JOIN ( 
SELECT MAX(TransactionDate) AS TransactionDate,paitientcode FROM (
SELECT MAX(saledate) AS TransactionDate ,paitientcode FROM salemaster
GROUP BY paitientcode 
UNION SELECT MAX(billdate) AS saleid,paitentid FROM consultingbillmaster
GROUP BY paitentid) AS a
GROUP BY paitientcode) AS f ON d.`paitentid`=f.paitientcode

WHERE   (topay-receipt) > 1 order by f.TransactionDate desc
...
echo " <thead><tr>  
		...
		<th width='%' hidden> Location Name </th>    
...
$result = mysqli_query($connection, " 
  ...
  GROUP BY paitientcode ORDER BY 1 DESC) AS b ON a.saleid=b.saleid 
  JOIN locationmaster AS c ON a.locationcode =c.locationcode
  JOIN paitentmaster AS d ON a.paitientcode=d.paitentid AND (topay-receipt) > 1
