...
$query=mysqli_query($connection, " SELECT(
 SELECT ROUND(IFNULL(SUM(amount),0),2) FROM `salepaymentdetails`  AS a JOIN  paymentmodemaster AS c ON a.paymentmode=c.paymentmodecode 
    WHERE transactiontype='Sales' AND c.paymentmode='Cash' AND DATE BETWEEN '$ActualFromDate' AND '$ActualToDate' and transactionstatus='Cancelled' and a.invoiceno in (
	SELECT saleuniqueno FROM salemaster AS a JOIN paitentmaster AS b ON 
	a.paitientcode = b.paitentid 
	JOIN usermaster as c ON a.adddby = c.userid
	WHERE cancellstatus ='1' and a.saledate BETWEEN '$ActualFromDate' AND '$ActualToDate'  
 and a.locationcode like('$Location')  ) ) AS Cash,
 
 (  SELECT ROUND(IFNULL(SUM(amount),0),2) FROM `salepaymentdetails`  AS a JOIN  paymentmodemaster AS c ON a.paymentmode=c.paymentmodecode 
    WHERE transactiontype='DoctorFee' AND c.paymentmode='CARD' AND DATE BETWEEN '$ActualFromDate' AND '$ActualToDate'  and transactionstatus='Cancelled' and a.invoiceno in (SELECT saleuniqueno FROM salemaster AS a JOIN paitentmaster AS b ON 
	a.paitientcode = b.paitentid 
	JOIN usermaster as c ON a.adddby = c.userid
	WHERE cancellstatus ='1' and a.saledate BETWEEN '$ActualFromDate' AND '$ActualToDate'  
 ...
 SELECT ROUND(IFNULL(SUM(amount),0),2) FROM `salepaymentdetails`  AS a JOIN  paymentmodemaster AS c ON a.paymentmode=c.paymentmodecode 
	...
	JOIN usermaster as c ON a.adddby = c.userid
	WHERE cancellstatus ='1' and  a.saledate BETWEEN '$ActualFromDate' AND '$ActualToDate'  
 and a.locationcode like('$Location')   ) ) AS Others,
	...
	JOIN usermaster as c ON a.adddby = c.userid
	WHERE cancellstatus ='1' and  a.saledate BETWEEN '$ActualFromDate' AND '$ActualToDate'  
 and a.locationcode like('$Location'))  ) AS Total
  "); 
	 $data = array();
