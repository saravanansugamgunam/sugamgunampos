<?php
  ...
  SELECT mappingid, 
  CONCAT(productshortcode,'<br> ' ,'<b>', 'Rack: ',IFNULL(Rack,'-') ), 
  ...
  left join (select ifnull(concat(rack,'-',shelf,'-',bin),'-') as Rack,barcode
   from rack_stockbinmapping ) as e
  on b.uniquebarcode = e.barcode
