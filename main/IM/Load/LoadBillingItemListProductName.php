...
SELECT saleid,productcode,concat(shortcode,'-',a.productname),batchcode,SUM(saleqty)  AS Qty, mrp*SUM(saleqty) AS Total, 
round(SUM(discountamount),0) AS Discount, round(SUM(nettamount),0) AS nett, currentstock,b.weight FROM newsaleitemsproduct as a 
join productmaster as b on a.productcode =b.productid  WHERE invoiceno ='$Invoice'
GROUP BY shortcode,a.productname,currentstock,batchcode,saleid 
...
echo " <thead><tr>  
		...
		<th   width='%'> <a href=\"javascript:SortTable(3,'T');\">Weight</a></th>    
		 <th width='%'>  Delete </th>   
