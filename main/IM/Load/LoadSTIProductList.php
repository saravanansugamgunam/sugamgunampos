 ...
 function LoadReturnItems() {
		...
		var Value = 0;
            ...
            $("#tblReturnItems input[type=checkbox]:checked").each(function () {
				...
				Value += parseFloat(row.cells[5].innerHTML);
...
$result = mysqli_query($connection, "
 ...
 SELECT itemid,shortcode,batchcode,SUM(stoqty)  AS Qty,  SUM(nettamount) AS nett FROM newstoitems WHERE stouniqueno ='$STOID'
 GROUP BY shortcode,productname,currentstock,batchcode,itemid
 ");
...
echo " <thead><tr>  
		<th></th>       
		<th>#</th>       
		<th  width='%'><a href=\"javascript:SortTable(2,'T');\">PCode</a></th>    
		<th width='%'><a href=\"javascript:SortTable(2,'T');\">SC</a></th>   
...
while($data = mysqli_fetch_row($result))
{
  echo "
  <tr>
  <td width='%'><input type='checkbox'/></td>
  <td width='10%'>$SerialNo</td>
  <td  > $data[0]</td>
  <td  width='%' >$data[1]</td>  
   <td width='%'>$data[2]</td>   
   <td width='%'>$data[3]</td>      
   <td id='TotalAmount' width='%'>$data[4]</td>     
...
echo "<br>"; 
    ...
    echo "<input style='border-radius: 4px; padding: 5px;  text-align: right;'  id='txtTotalReturn' name ='txtTotalReturn' />";
	 
	echo " <input id = 'btnGet' class='btn btn-sm btn-danger'  type='button' onclick='LoadReturnItems();' value='Confirm' />";
	echo "<input type ='hidden' id='txtItemId' name ='txtItemId' />";
	echo "<hr>";
