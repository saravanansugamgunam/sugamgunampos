...
if (isset($_POST["Type"])) {
	...
	SELECT ( 
		SELECT IFNULL(ROUND(SUM(nettamount+ couriercharges),0),0) AS Amount FROM salemaster  
		WHERE  saledate BETWEEN '$ActualFromDate' AND '$ActualToDate'   AND locationcode like('$Location') and 
		transactiontype <> 'Return' and cancellstatus = 0) AS TotalIncome,
		(SELECT COUNT(*) FROM salemaster WHERE  saledate BETWEEN '$ActualFromDate' AND '$ActualToDate'   AND locationcode like('$Location')  and cancellstatus = 0) AS TotalBills,
		...
		(SELECT SUM(nettamount + couriercharges)-SUM(received) + sum(oldbalance) FROM salemaster WHERE locationcode like('$Location') and 
		saledate BETWEEN '$ActualFromDate' AND '$ActualToDate'  and cancellstatus = 0) as TodayOverdue,
 ...
 "); 
  ...
  <tr><td>Today Return</td> <td style='text-align:right;' > $data[7]</td> </tr>   
...
<i class='fa fa-2x fa-eye' title='View' style='color:orange;' onclick='LoadBillDetails();'  ></i></a>
  ...
  <tr><td>Today's Outstanding.</td> <td style='text-align:right;' > $data[6]</td> </tr>    
	...
	$result = mysqli_query($connection, " 
SELECT b.username, IFNULL(ROUND(SUM(nettamount),0),0) AS Amount  FROM salemaster AS a JOIN usermaster as b ON 
a.doctorcode = b.userid
