...
$result = mysqli_query($connection, " 
 SELECT DATE_FORMAT(saledate,'%d-%m-%Y'), b.paitentname,b.mobileno, ROUND(SUM(received),0),d.paymentmode
    FROM salemaster  AS a
 JOIN paitentmaster AS b ON a.paitientcode=b.paitentid
 JOIN salepaymentdetails  AS c ON a.saleuniqueno=c.invoiceno
 JOIN paymentmodemaster AS d ON c.paymentmode=d.paymentmodecode
 WHERE saledate BETWEEN '$ActualFromDate' AND '$ActualToDate'  and a.transactiontype in('Outstanding')  and a.locationcode like('$Location')  GROUP BY DATE_FORMAT(saledate,'%d-%m-%Y'), b.paitentname,b.mobileno ,d.paymentmode
  ORDER BY 1 
 ");

// echo $result;
 //echo "<table id='tblProject' class='tblMasters'>";
  echo "  <table id='data-table' class='table table-striped table-bordered' style='width:90%;'>";
echo " <thead><tr>  
		<th>S.No</th>              
		<th  width='%'> Date</a></th>    
		<th  width='%'> Patient  </th>      
		<th  width='%'> Mobile No  </th>      
		<th class='count-me' width='%'> Received Amount </th>
		<th class='count-me' width='%'> Payment Mode </th>
...
$result = mysqli_query($connection, " 
 SELECT saleuniqueno,CONCAT(a.invoiceno,'-',a.saleid) AS Bill,DATE_FORMAT(a.saledate,'%d-%m-%Y'), b.paitentname, 
 ...
 WHERE a.saledate BETWEEN '$ActualFromDate' AND '$ActualToDate' and a.locationcode ='$LocationCode'  and a.transactiontype  in('Outstanding') ");
...
while($data = mysqli_fetch_row($result))
{
  ...
  <td align='center' style='color:#009ad9' >";
	...
	{
		echo "<button  class='btn btn-sm btn-danger btn-xs' data-toggle='modal' data-target='#myModalCancel' 
	onclick='LoadItemDetails(".$data[0].");' >Return</button>";
	}
...
$result = mysqli_query($connection, "  
 ...
 WHERE saledate BETWEEN '$ActualFromDate' AND '$ActualToDate'  and locationcode ='$LocationCode'  and a.transactiontype  in('Outstanding')
  GROUP BY locationcode ,DATE_FORMAT(saledate,'%d-%m-%Y') ");

 //echo "<table id='tblProject' class='tblMasters'>";
  echo "  <table id='yy' class='table table-striped table-bordered'>";
echo " <thead><tr>  
		<th>S.No</th>          
