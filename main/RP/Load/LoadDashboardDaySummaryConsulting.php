...
if (isset($_POST["Type"])) {
	...
	$result = mysqli_query($connection, " 
				...
				SELECT  
					...
					where  billdate BETWEEN '$ActualFromDate' AND '$ActualToDate'   AND locationcode like('$Location') 
					 and cancelledstatus ='0' and consultationuniquebill not in (select bookinguniqueid from therapybookingmaster ) ) AS TotalIncome, 
					...
					(SELECT COUNT(*) FROM consultingbillmaster WHERE  billdate BETWEEN '$ActualFromDate' AND '$ActualToDate' 
					  ...
					  and consultationuniquebill not in (select bookinguniqueid from therapybookingmaster )) AS TotalBills,

					(SELECT COUNT(DISTINCT paitentid) FROM consultingbillmaster WHERE  billdate BETWEEN '$ActualFromDate' AND '$ActualToDate'
					AND locationcode like('$Location') and cancelledstatus ='0' 
					and consultationuniquebill not in (select bookinguniqueid from therapybookingmaster )) AS TotalPaitent,
					...
					AND '$ActualToDate 23:59:59') and consultationuniquebill not in (select bookinguniqueid from therapybookingmaster )) AS NewPaitent,

					(SELECT COUNT(*) FROM consultingbillmaster WHERE  billdate BETWEEN '$ActualFromDate' AND '$ActualToDate'  
					 AND locationcode like('$Location') and cancelledstatus ='0' AND discountamount > 0 and 
					 consultationuniquebill not in (select bookinguniqueid from therapybookingmaster )) AS DiscountBill,
					...
					consultationuniquebill not in (select bookinguniqueid from therapybookingmaster )) AS DiscountAmount,


					(select ifnull(sum(outstandingamount),0) as Outstandig from outstandingdetails where transactiontype='Consulting - Sale' and 
 ...
 ");
  ...
  <tr><td>Disc. Bills <a href='' data-toggle='modal' data-target='#ModalBillDetailsConsulting'  style='float: right; '>
  <i class='fa fa-2x fa-eye' title='View' style='color:orange;' onclick='LoadBillDetailsConsulting();'  ></i></a>
...
WHERE   transactionstatus='Live' AND transactiontype IN('DoctorFee') 
...
consultationuniquebill not in (select bookinguniqueid from therapybookingmaster )
