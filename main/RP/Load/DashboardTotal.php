...
(SELECT  IFNULL(ROUND(SUM(amount),0),0) FROM salepaymentdetails WHERE
 transactiontype IN('CashAdvance','Therapy PaymentAdvance','IncomeEntry','PaitentOrder',
 'OutstandingCollection','Sales','Therapy Payment','DoctorFee','Diagnosis','Advance - PaitentOrder')
...
// (SELECT IFNULL(ROUND(SUM(amount),0),0) FROM salepaymentdetails WHERE transactiontype IN('PaitentOrder','CashAdvance','Therapy PaymentAdvance')  and date BETWEEN '$ActualFromDate' AND '$ActualToDate'  and clientid like ('$Location')) AS Advance,
...
// ( SELECT  IFNULL(ROUND(SUM(topay)-SUM(receipt),0),0) FROM paitentmaster ) AS Outstanding,
// (SELECT  IFNULL(ROUND(SUM(amount),0),0) FROM salepaymentdetails WHERE transactiontype IN('PaitentOrder','CashAdvance','Therapy PaymentAdvance','Sales') and clientid like ('$Location') and date BETWEEN '$ActualFromDate' AND '$ActualToDate' and clientid like ('$Location') ) AS TotalIncome,
