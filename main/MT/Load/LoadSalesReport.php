...
//     where billtype='Consultation' billdate BETWEEN '$ActualFromDate' AND '$ActualToDate'
//     GROUP BY  a.billdate, b.`referenceno`,COUNT(DISTINCT b.`paitentid`),CONCAT(b.paitentname,' - ',b.mobileno) 
...
// "); 
    ...
    SELECT 1 $Query, SUM(Totalpaitent),SUM(bills),SUM(amount) AS Amount FROM (
        ...
        SELECT a.`saledate`, c.`reference`,COUNT(DISTINCT b.`paitentid`) AS Totalpaitent,CONCAT(b.paitentname,' - ',b.mobileno) AS Paitent ,
        COUNT(a.`saleid`) AS bills ,SUM(nettamount) AS Amount,'Inventory' AS transactiontype FROM 
        salemaster AS a JOIN paitentmaster AS b ON a.`paitientcode`=b.`paitentid`  LEFT JOIN referencemaster AS c ON b.`referenceid`=c.`referenceid` 
        WHERE  saledate BETWEEN '$ActualFromDate' AND '$ActualToDate'
        GROUP BY a.`saledate`, c.`reference`, CONCAT(b.paitentname,' - ',b.mobileno)  
        
        UNION
        
        
SELECT bookingdate,c.`reference`,COUNT(DISTINCT b.`paitentid`),CONCAT(b.paitentname,' - ',b.mobileno) ,
 COUNT(a.`bookingid`) ,SUM(fees),'Therapy' AS transactiontype
 FROM therapybookingmaster AS a JOIN paitentmaster AS b 
ON a.`paitentid`=b.`paitentid` LEFT JOIN referencemaster AS c ON b.`referenceid`=c.`referenceid` 
WHERE bookingdate BETWEEN '$ActualFromDate' AND '$ActualToDate'
GROUP BY bookingdate,c.`reference`, CONCAT(b.paitentname,' - ',b.mobileno) 
 
 UNION
 
 
    SELECT a.billdate, c.`reference`,COUNT(DISTINCT b.`paitentid`),CONCAT(b.paitentname,' - ',b.mobileno) ,
        COUNT(a.`billid`) ,SUM(totalamount),'Consulting' AS transactiontype  FROM 
        consultingbillmaster AS a JOIN paitentmaster AS b ON a.`paitentid`=b.`paitentid` 
        LEFT JOIN referencemaster AS c ON b.`referenceid`=c.`referenceid` 
        WHERE billtype='Consultation' and billdate BETWEEN '$ActualFromDate' AND '$ActualToDate'
        GROUP BY  a.billdate, c.`reference`, CONCAT(b.paitentname,' - ',b.mobileno)  
