...
c.paymentmode,a.remarks FROM referencesharedetails AS a JOIN
referencemaster AS b ON a.referenceid=b.referenceid 
LEFT JOIN `referencesharebilldetails` AS e ON a.`invoiceno`=e.`paymentid`
...
LEFT JOIN paitentmaster AS g ON f.`paitentid`=g.`paitentid`
 JOIN paymentmodemaster AS c ON a.paymentmode=c.paymentmodecode WHERE a.sharefor IN('Therapy')
  and a.entrydate between '$FromDate' and '$ToDate' and a.referenceid='$Doctor'
...
echo " <thead><tr>  
       ...
       <th width='%' hidden> For</th>    
       <th width='%' hidden> Period</th>   
       <th width='%' hidden> Paitent</th>   
        <th width='%'>Paid</th>    
       <th width='%'>Mode</th>     
       <th width='%'>Remarks</th>        
...
while($data = mysqli_fetch_row($result))
{
 ...
 <td> $data[0]</td>
 <td >$data[1]</td>  
  <td  width='%' hidden>$data[2]</td>   
  <td width='%' hidden>$data[3]</td>     
  <td width='%' hidden>$data[4]</td>     
  <td width='%' >$data[5]</td>      
  <td width='%'>$data[6]</td>     
  <td width='%'>$data[7]</td>       
