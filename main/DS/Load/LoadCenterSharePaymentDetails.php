...
c.paymentmode,a.remarks FROM diagnosticcentersharedetails AS a JOIN
diagnosticcentre AS b ON a.centercode=b.centerid 
LEFT JOIN `doctorsharebilldetails` AS e ON a.`invoiceno`=e.`paymentid`
LEFT JOIN diagnosissalemaster AS f ON e.`billuniqueid`=f.`diagnosisuniqueno`
LEFT JOIN paitentmaster AS g ON f.`paitentid`=g.`paitentid`
 JOIN paymentmodemaster AS c ON a.paymentmode=c.paymentmodecode WHERE a.sharefor IN('Samples')
 and a.entrydate between '$FromDate' and '$ToDate' and a.centercode='$CenterID'
