 ...
 if($Device=='Mobile')
 {
  ...
  try {
    if($PaitentID>0)
    {
      ...
      referenceid='$ReferenceCode'   where paitentid='$PaitentID'"; 
   
    }
    else
    {
      $AddPaymentMode = "insert into paitentmaster (paitentname ,mobileno,email,whatsappno,alternateno,referenceno,
      address,dob,gender,barcode,city,statecode,pincode,device,signatureimage,referenceid,activestatus) values
       ('$Name','$MobileNo','$PatientEmail','$Whatsapp',
      '$AlternateNo','$ReferenceNo','$Address','$DOB','$Sex','$PatientBarcode',
      '$City','$State','$Pincode','$Device','$Signature','$ReferenceCode','$ActiveStatus')"; 
    }
