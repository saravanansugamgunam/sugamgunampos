...
JOIN  consultationmaster AS d ON c.therapyid=d.consultationid 
 ...
 d.consultationname,c.totalsitting,a.transactionmode ,0,0,a.creditamount,0,a.vendorcode
  ...
  JOIN therapybookingmaster AS c ON a.invoicegrn=c.bookinguniqueid 
 JOIN  consultationmaster AS d ON c.therapyid=d.consultationid 
  WHERE transactiontype='Therapy' AND transactionmode='Therapy - Cancellation'  and 
  bookingdate between   '$FromPeriod' and '$ToPeriod' and vendorcode like '$SupplierCode' 
   UNION
...
JOIN  consultationmaster AS d ON c.therapyid=d.consultationid 
 ...
 paitentname,consultationname,totalsitting,vendorcode ORDER BY 12 DESC
...
echo " <thead><tr>  
    ...
    <th width='%'> <a href=\"javascript:SortTable(3,'T');\">Modify Mode</a></th>  
    <th width='%'> <a href=\"javascript:SortTable(3,'T');\">Payment</a></th>  
...
while ($data = mysqli_fetch_row($result)) {
  ...
  echo "</td>";
  echo "<td width='%' align='right'>";
  echo formatMoney($data[10], false);
  echo "</td>";
  echo "<td width='%' align='right'>";
  echo formatMoney($data[11], false);
  echo "</td>
...
<td align='center' style='color:#009ad9'>
    <a href='#modlPaymentModeModification' data-toggle='modal' onclick='LoadPaymentModedetails($data[0]);'>
...
<td bgcolor='#c3602c' align='center'>
    <button class='btn  m-r-5' href='#ModalPaymentDetails' data-toggle='modal'
        onclick='GetID($data[0],$data[12],$data[11]);'> Collect </button>";
    }
