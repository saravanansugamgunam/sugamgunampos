...
IFNULL(d.timeslot,'Open'), b.consultationname, 
 CONCAT(g.`paitentname`,'<br>' ,g.`mobileno`) AS Paitent,   
...
LEFT JOIN (SELECT bookingitemid,timeslot AS timeslot,c.timevalue FROM timeslotallocation AS a 
...
JOIN usermaster AS e ON a.referedbydoctorid=e.userid   
  ...
  where a.`reviseddate` between '$FromPeriod' and '$ToPeriod' and
  a.doctorid like ('$Therapist') and a.bookingstatus not in ('Cancelled')
...
ORDER BY reviseddate ,timevalue
...
echo " <thead><tr>  
		...
		<th   width='%' > Therapy&nbsp;Dt </th>    	
		<th nowrap width='%'>Therapist</th>   
		<th nowrap width='%'>Time</th> 
		<th nowrap width='%'>Therapy</th>  
		<th nowrap width='%'>Paitent</th> 
		<th nowrap width='%'>Status</th> 
		<th nowrap width='%'>Closing Date</th> 
		<th nowrap width='%'>Balance</th> 
