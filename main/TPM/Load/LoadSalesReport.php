...
// 	echo "SELECT CONCAT(DATE_FORMAT(a.billdate,'%d-%m-%Y'),' ', 
//     DATE_FORMAT(DATE_ADD(DATE_ADD(a.billaddedon ,INTERVAL 5 HOUR) ,INTERVAL 30 MINUTE),'%h:%i %p')) ,a.consultationuniquebill,a.billid, 
//   CONCAT(b.paitentname,'(',b.mobileno,')') AS paitent,
//   c.username,a.totalamount+a.extracharge,cancelledstatus,
// 	a.receivedamount,SUM(d.discount) AS discount,billtype FROM `consultingbillmaster` AS a JOIN paitentmaster AS b
...
// and f.paymentmode like('$PaymentMode') 
...
echo " 
SELECT CONCAT(DATE_FORMAT(a.billdate,'%d-%m-%Y'),' ', 
DATE_FORMAT(DATE_ADD(DATE_ADD(a.billaddedon ,INTERVAL 5 HOUR) ,INTERVAL 30 MINUTE),'%h:%i %p')) ,a.consultationuniquebill,a.billid, 
CONCAT(b.paitentname,'(',b.mobileno,')') AS paitent,
c.username,a.totalamount+a.extracharge,cancelledstatus,
  a.receivedamount,SUM(d.discount) AS discount,billtype FROM `consultingbillmaster` AS a JOIN paitentmaster AS b
  ON a.paitentid = b.paitentid  
JOIN usermaster as c ON a.doctorid = c.userid join 
therapybookingdetails as d on a.consultationuniquebill = d.bookinguniqueid
join consultationmaster as e on d.therapyid=e.consultationid
left join salepaymentdetails as f on a.consultationuniquebill = f.invoiceno
WHERE  a.billdate BETWEEN '$ActualFromDate' AND '$ActualToDate' AND cancelledstatus='0'
and a.locationcode like('$Location')  and d.therapyid like('$ConsultationType') 
and  e.consultingtype like ('$ConsultationGroup') and a.doctorid like ('$Doctor')
and f.paymentmode like('$PaymentMode') 
GROUP BY CONCAT(DATE_FORMAT(a.billdate,'%d-%m-%Y'),' ', 
DATE_FORMAT(DATE_ADD(DATE_ADD(a.billaddedon ,INTERVAL 5 HOUR) ,INTERVAL 30 MINUTE),'%h:%i %p')) ,a.consultationuniquebill,a.billid, CONCAT(b.paitentname,
...
// WHERE a.saledate BETWEEN '$ActualFromDate' AND '$ActualToDate' and a.locationcode ='$LocationCode' and
...
NOT IN('Outstanding','Cancelled') AND cancellstatus =0 and
...
//echo "<table id='tblProject' class='tblMasters'>";
    ...
    a.transactiontype not in('Outstanding') and cancellstatus =0 and
    saletype like ('%" . $BillMode . "%')
