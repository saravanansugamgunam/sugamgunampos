...
$result = mysqli_query($connection, "
SELECT a.bookingid,a.therapyid,b.consultationname,a.totalsitings,
CONCAT(DATE_FORMAT(reviseddate,'%d-%m-%Y'),' : ',LEFT(DATE_FORMAT(reviseddate,'%W'),3)),

IFNULL(d.timeslot,revisedtime) AS revisedtime,
...
left JOIN 
(SELECT bookingitemid,CONCAT(timeslot,' - ',SUBSTR(SEC_TO_TIME(round(COUNT(timeslot)/2,1)*60),4),'Hr') AS timeslot FROM timeslotallocation_temp AS a 
...
echo " <thead><tr>  
		<th>S. No</th>     
		<th width='%' hidden><a href=\"javascript:SortTable(2,'T');\">ID</a></th>        
		<th width='%' hidden><a href=\"javascript:SortTable(2,'T');\">Therapy ID</a></th>        
