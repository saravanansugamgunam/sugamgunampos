...
<tr>  
		...
		<th nowrap width='%'> Today's Income</th>        
		<th nowrap width='%'> Today's Expense</th>        
		<th nowrap width='%'> Today's Closing Balance </th>             
		 </tr>
  </thead> <tbody>";
...
$result = mysqli_query($connection, "  
 SELECT a.ledgerid, DATE_FORMAT(a.date, '%d-%m-%y') AS DATE ,c.categoryname,b.ledgername,incomeamount,expenseamount, (incomeamount-expenseamount) AS Total   FROM accountingtransaction AS a JOIN accountingledger AS b ON a.ledgerid=b.ledgerid 
 JOIN accoutingcategory AS c ON b.categoryid=c.categoryid  WHERE  DATE_FORMAT(a.date, '%Y-%m-%d') = '$currentdate' and transactiongroup='Clinic'");

 //echo "<table id='tblProject' class='tblMasters'>";
  echo "  <table id='data-table' class='table table-striped table-bordered'>";
echo " <thead><tr>  
		<th>S.No</th>          
		<th hidden width='%'><a href=\"javascript:SortTable(2,'T');\">ID</a></th>    
		<th  width='%'><a href=\"javascript:SortTable(2,'T');\">Date</a></th>    
		<th  width='%'><a href=\"javascript:SortTable(2,'T');\">Category</a></th>     
		<th width='%'> <a href=\"javascript:SortTable(3,'T');\">Ledger</a></th>           
		<th width='%'> <a href=\"javascript:SortTable(3,'T');\">Income</a></th>        
		<th width='%'> <a href=\"javascript:SortTable(3,'T');\">Expense</a></th>        
...
echo "<td width='%' align='right'>"; echo formatMoney($data[4], false); echo "</td>
 <td width='%' align='right'>"; echo formatMoney($data[5], false); echo "</td>
 <td width='%' align='right'>"; echo formatMoney($data[6], false); echo "</td>   
