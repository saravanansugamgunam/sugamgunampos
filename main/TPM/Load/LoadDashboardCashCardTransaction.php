...
WHERE  a.`date` BETWEEN '$ActualFromDate' AND '$ActualToDate'  and a.clientid like ('$Location')   
 AND transactionstatus ='Live'  and transactiongroup ='Clinic' 
  and a.transactiontype in ('Therapy Payment','IncomeEntry','DoctorFee')
...
ON a.`paymentmode`=b.`paymentmodecode`
WHERE  a.`date` BETWEEN '$ActualFromDate' AND '$ActualToDate'  and a.clientid like ('$Location')   
 AND transactionstatus ='Live'  and transactiongroup ='Clinic' and a.transactiontype in ('ExpenseEntry','Salary','Supplier Payment','Therapy Share')
