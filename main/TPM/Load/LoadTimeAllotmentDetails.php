...
SELECT a.timeslot,a.id,bookdestatus, b.timeslot,b.id ,bookdestatus1,a.starttime FROM ( 

  SELECT a.id,a.timeslot,IFNULL(SUM(totaltime),0) AS AllotedTime, ROUND((IFNULL(SUM(totaltime),0)/60)*100,0) AS TotalPercent, 
  row_number() over (ORDER BY id) AS seqnum,CASE
      WHEN bookedstatus = 1 THEN 'Booked'
      WHEN bookedstatus = 2 THEN 'Recomended'
      ELSE 'Open'
  END AS bookdestatus,starttime
  
  FROM timeslotdetails AS a LEFT JOIN 
  (SELECT timeslotid,totaltime FROM ( SELECT timeslotid,totaltime FROM timeslotallocation 
  UNION 
  SELECT timeslotid,totaltime FROM 
  timeslotallocation_temp WHERE uniqueid ='$Invoice') AS a GROUP BY timeslotid,totaltime) AS b ON a.id =b.timeslotid 
  ...
  ON a.uniqueid=b.bookinguniqueid) AS d ON a.id = d.timeslotid
  
  WHERE a.assigneddate ='$TherapyDate' AND a.doctorid='$Doctor' AND a.starttime < 14 GROUP BY a.id,a.timeslot ORDER BY starttime) AS a 
  ...
  row_number() over (ORDER BY id) AS seqnum ,CASE
      ...
      WHEN bookedstatus = 2 THEN 'Recomended'
      ELSE 'Open'
  END AS bookdestatus1 FROM timeslotdetails AS a LEFT JOIN 
  (SELECT timeslotid,totaltime FROM ( SELECT timeslotid,totaltime FROM timeslotallocation 
  UNION 
  SELECT timeslotid,totaltime FROM 
  timeslotallocation_temp WHERE uniqueid ='$Invoice') AS a GROUP BY timeslotid,totaltime) AS b ON a.id =b.timeslotid 
...
while($data = mysqli_fetch_row($result))
{
 ...
 else  if($data[2]=='Recomended')
 {
  echo "<td bgcolor='#f6f2ab'  width='%'    onclick='GetTimeSlotMorning(this);' style='cursor: pointer' >
  <input class='checkbox-round'   type='checkbox'/ onclick='LoadMorningID();'>
  $data[2]
  
  </td>  ";
 }
 else  if($data[2]=='Booked')
 {
...
$result = mysqli_query($connection, " 
SELECT a.timeslot,a.id,bookdestatus, b.timeslot,b.id ,bookdestatus1,b.starttime FROM ( 
  ...
  row_number() over (ORDER BY id) AS seqnum ,CASE
      ...
      WHEN bookedstatus = 2 THEN 'Recomended'
      ELSE 'Open'
  END AS bookdestatus1,starttime FROM timeslotdetails AS a LEFT JOIN 
  (SELECT timeslotid,totaltime FROM ( SELECT timeslotid,totaltime FROM timeslotallocation 
  UNION 
  SELECT timeslotid,totaltime FROM 
  timeslotallocation_temp WHERE uniqueid ='$Invoice') AS a GROUP BY timeslotid,totaltime) AS b ON a.id =b.timeslotid 
  ...
  WHERE a.assigneddate ='$TherapyDate' AND a.doctorid='$Doctor' AND a.starttime > 13 GROUP BY a.id,a.timeslot ORDER BY starttime) AS b 
...
while($data = mysqli_fetch_row($result))
{
 ...
 <tr>
  ...
  if($data[5]=='Open')
  {
   echo "<td bgcolor='#7cd342'  width='%'    onclick='GetTimeSlotEvening(this);' style='cursor: pointer' >
   <input class='checkbox-round'   type='checkbox'/ onclick='LoadEveningID();'>
   $data[5]
   
   </td>  ";
  }
  else  if($data[5]=='Recomended')
  {
   echo "<td bgcolor='#f6f2ab'  width='%'    onclick='GetTimeSlotEvening(this);' style='cursor: pointer' >
   <input class='checkbox-round'   type='checkbox'/ onclick='LoadEveningID();'>
   $data[5]
   
   </td>  ";
  }
  else  if($data[5]=='Booked')
  {
   echo "<td bgcolor='#ff8f6b' width='%'>Booked</td> ";
  }
