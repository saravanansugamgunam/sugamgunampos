...
//    $position=$_SESSION["SESS_LAST_NAME"]; 
    ...
    $RecomendationID=$_GET['RD'];
 

    $res = $connection->query(" 
    SELECT
    (SELECT userid FROM usermaster WHERE userid IN
    (SELECT addedby FROM therapyrecomendation WHERE currentstatus='Pending' AND   uniqueid='$RecomendationID')) AS ReferedbyDoctor,
    (SELECT paitentid FROM therapyrecomendation WHERE currentstatus='Pending' AND  uniqueid='$RecomendationID' LIMIT 1 ) AS Paitentid,
    (SELECT CONCAT(paitentname,'-',mobileno) AS Paitent FROM paitentmaster  WHERE paitentid 
    IN(SELECT paitentid FROM therapyrecomendation WHERE currentstatus='Pending' AND  uniqueid='$RecomendationID' GROUP BY paitentid)) AS Paitent,
    (SELECT doctorid FROM therapyrecomendation WHERE currentstatus='Pending' AND uniqueid='$RecomendationID' LIMIT 1 ) AS Doctor,
    (SELECT recomendeddate FROM therapyrecomendation WHERE currentstatus='Pending' AND uniqueid='$RecomendationID' LIMIT 1 ) AS RecomendedDate"); 
...
<body onload="LoadInvoiceNo();" onmousedown="CalculateCurrentTotal()">
    ...
    <div id="page-container" class="fade page-sidebar-minified page-header-fixed">
        ...
        function SaveTherapyDetails() {
            ...
            if (PaitentName == "" || Doctor == "" || DoctorReferedby == "" || TherapyName == "" || BookingDate == "" ||
                InvoiceNo == "" ||
                ...
                var datas = "&PaitentName=" + PaitentName + "&TherapyName=" + TherapyName + "&BookingDate=" +
                    ...
                    "&EntryType=" + EntryType + "&TotalHours=" + TotalHours + "&EveningTimeSlotID=" +
                    EveningTimeSlotID +
        ...
        function SaveBillingDetails() {
            ...
            // alert(MorningTimeSlotID);
            if (Invoice == "" || TotalSaleAmount == "" || PaitentCode == "" || SaleDate == "" || TherapyDate == "" ||
                EveningTimeSlotID == "" && MorningTimeSlotID == "" && WaitingList == "" || DoctorCode == "" ||
                EntryType == "" ||
                DoctorReferedby == "" || TotalSaleAmount == "0") {
            ...
            <div class="modal fade" id="ModalSchedule" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-warning" onclick='SetWaitingList()'
                                data-dismiss="modal" style='float:left'>Waiting List</button>
...
<script>
                function LoadAutoDetails() {
                    ...
                    document.getElementById("cmbDoctorReferedby").value = <?php echo $ReferedBy; ?>;
                    document.getElementById("cmbPaitent").value = "<?php echo $Paitentid; ?>";
                    document.getElementById("cmbPaitentName").value = "<?php echo $PaitentName; ?>";
                    document.getElementById("cmbDoctor").value = <?php echo $Doctor; ?>;
                    document.getElementById("dtTherapyDate").value = "<?php echo $RecomendedDate; ?>";
                    document.getElementById("dtTherapyTime").value = "<?php echo $currentdate; ?>";
                    document.getElementById("txtPaitentCode").value = "<?php echo $Paitentid; ?>";
                      var Paitentid = <?php echo $Paitentid; ?>;
                      
                    LoadPaitentDetails(Paitentid);
                    ImportTherapyDetails();



                }

                function ImportTherapyDetails() {
            var Invoice = document.getElementById("txtInvoiceNo").value;
             var RecomendedID = '<?php echo $RecomendationID; ?>';  
            var BookingDate = '<?php echo $RecomendedDate; ?>'; 
            var BookingTime = '<?php echo $currentdate; ?>';   
            var ReferedBy = <?php echo $ReferedBy; ?>;   

            var datas = "&Invoice=" + Invoice 
                      + "&RecomendedID=" + RecomendedID
                      + "&BookingDate=" + BookingDate
                      + "&ReferedBy=" + ReferedBy
                      + "&BookingTime=" + BookingTime;
            // alert(datas);
            $.ajax({
                url: "Save/ImportRecomendedList.php",
                method: "POST",
                data: datas,
                success: function(data) {
                ...
                <div class="panel panel-inverse">
                    <div class="panel-body">
                        ...
                        <input type="hidden" name="txtDoctorEmail" id="txtDoctorEmail" placeholder=""
                            class="form-control" />
                        <input type="hidden" name="txtmobileno" id="txtmobileno" placeholder=""
                            class="form-control" />
                        ...
                        <div>
                            <table>
                                ...
                                <tr>
                                    ...
                                    <td><select class="form-control"
                                            ...
                                            <?php  
                            $sqli = "SELECT userid,username FROM usermaster where designationid='9'  ";
                            $result = mysqli_query($connection, $sqli); 
                             while ($row = mysqli_fetch_array($result)) {
                            	# code...
                            ...
                            ?>
                                    ...
                                    <td>&nbsp;&nbsp;</td>
                                    <td>
                                           <input type="hidden" name="cmbPaitent" id="cmbPaitent" class="form-control"
                                            disabled style='width: 350px;' />

                                            <input type="text" name="cmbPaitentName" id="cmbPaitentName" class="form-control"
                                            disabled style='width: 350px;' />
                            ...
                            ?>
                                    ...
                                    <td><input type="date" class="form-control" placeholder="" id='dtTherapyDate'
                                            name='dtTherapyDate' value='<?php echo date('Y-m-d'); ?>' /></td>

                                    <td>
                                        <input type="text" class="form-control" placeholder="Select Schedule"
                                            id='dtTherapyTime' name='dtTherapyTime' onclick="LoadAvailability()"
