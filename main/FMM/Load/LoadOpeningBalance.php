...
(SELECT IFNULL(ROUND(SUM(amount)-0,0),0) AS Amount FROM salepaymentdetails 
AS a 
JOIN transactionmasterreport AS b ON a.`transactiontype`=b.transactionname 
 WHERE transactionstatus ='Live' AND paymentmode ='12' AND b.`transactiontype`='Income'  
   AND DATE > 
   (SELECT CONCAT(MAX(closingdate), ' 23:59:59') FROM dayclosedetails where clientid ='$Location')  AND DATE <= '$DayCloseDate'  and clientid ='$Location' ) AS Income,
 
(SELECT IFNULL(ROUND(SUM(amount),0),0) AS Amount FROM salepaymentdetails 
AS a 
JOIN transactionmasterreport AS b ON a.`transactiontype`=b.transactionname 
 WHERE transactionstatus ='Live' AND paymentmode ='12' AND b.`transactiontype`='Expense'
   AND DATE > 
   (SELECT CONCAT(MAX(closingdate), ' 23:59:59') FROM dayclosedetails where clientid ='$Location')  AND DATE <= '$DayCloseDate'  and clientid ='$Location' ) AS Expense,
...
");
    ...
    {  
      $data[] = $row['Opening'];
	  $AvailablaeBalance = $row['Income'] -$row['Expense'] ;
