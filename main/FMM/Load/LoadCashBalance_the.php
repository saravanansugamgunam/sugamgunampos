...
WHERE  a.`date` = '$DayCloseDate' AND a.clientid LIKE ('$Location')  AND transactionstatus ='Live' 
 ...
 closingdate IN(SELECT MAX(closingdate) FROM denomination WHERE closinggroup='2' ) and  closinggroup='2')  as Opening
...
");
	...
	while ($row = mysqli_fetch_assoc($query)) {
		$data[] = $row['Consulting'] * 1 + $row['Therapy'];
		$data[] = $row['IncomeEntry'] * 0;
		$data[] = $row['ExpenseEntry'] * 0;
		$data[] = $row['Consulting'] + $row['Therapy'];
		$data[] = $row['Opening'];
	}
