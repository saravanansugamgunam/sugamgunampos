...
if (isset($_POST["InvoiceNo"])) {
  ...
  try {
    ...
    $SaveSaleMaster = "insert into doctorsharedetails (invoiceno,doctorcode,entrydate,fromdate,todate,calculationmode,profitpercent,
  ...
  '$TotalSale','$Profit','$PaymentMode','$Remarks','$userid','$ShareFor');";
   ...
   SELECT '$InvoiceNo',bookingid,'Therapy',referedbydoctorid,'$EntryDate',
   ROUND((SUM(nettamount)) *$SharePercentToCalculateBillwise,1) ,'$PaymentMode','$userid' FROM therapybookingdetails WHERE
   bookingid NOT IN (SELECT billuniqueid FROM doctorsharebilldetails where billtype ='Therapy') 
     and referedbydoctorid='$DoctorCode' and  closingdate   BETWEEN '$FromDate' AND '$ToDate'  AND   bookingstatus ='Closed'
     group by  bookingid,referedbydoctorid   ;";
        } else {
          $SaveSaleMaster .= "INSERT INTO doctorsharebilldetails (paymentid,billuniqueid,billtype,doctorid,paiddate,paidamount,paymentmode,createdby)
   ...
   SELECT '$InvoiceNo',bookingid,'Therapy',doctorid,'$EntryDate',
   ROUND((SUM(nettamount)) *$SharePercentToCalculateBillwise,1) ,'$PaymentMode','$userid' FROM therapybookingdetails WHERE
   bookingid NOT IN (SELECT billuniqueid FROM doctorsharebilldetails where billtype ='Therapy') 
     and doctorid='$DoctorCode' and  closingdate   BETWEEN '$FromDate' AND '$ToDate'  AND   bookingstatus ='Closed'
     group by  bookingid,doctorid   ;";
        } else {
          $SaveSaleMaster .= "INSERT INTO doctorsharebilldetails (paymentid,billuniqueid,billtype,doctorid,paiddate,paidamount,paymentmode,createdby)
