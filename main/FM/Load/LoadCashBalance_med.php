...
WHERE  a.date = '$DayCloseDate'  AND a.clientid LIKE ('$Location') AND
 ...
 AND b.paymentmodecode  = '12' AND transactiongroup in('Inventory')  
  ) AS IncomeEntry,
...
WHERE  a.`date` = '$DayCloseDate' AND a.clientid LIKE ('$Location')  AND transactionstatus ='Live' 
 AND transactiongroup ='Inventory' AND transactiontype IN ('DoctorFee','Diagnosis')
...
ON a.`paymentmode`=b.`paymentmodecode`
WHERE  a.`date` = '$DayCloseDate' AND a.clientid LIKE ('$Location')  AND transactionstatus ='Live' 
 AND transactiongroup ='Inventory' AND transactiontype IN ('Therapy Payment')
  AND transactionstatus='Live' 
 AND b.paymentmodecode  = '12'  AND 
 ...
 createdon IN (SELECT MAX(createdon) FROM  denomination   ) AND closinggroup='1' ) 
 and  closinggroup='1')  as Opening
...
");
	...
	while ($row = mysqli_fetch_assoc($query)) {
		...
		$data[] = $row['Sales'] +  $row['IncomeEntry'] - $row['ExpenseEntry'];
		$data[] = $row['Opening'];
	}
