 ...
 ");
   ...
   AND closingdate BETWEEN '$FromDate' AND '$ToDate'   AND referedbydoctorid LIKE ('$userid')  AND   bookingstatus ='Closed' 
   and bookingid not in (SELECT billuniqueid FROM doctorsharebilldetails where billtype ='Therapy') ");
        } else {
          $result = mysqli_query($connection, "SELECT ROUND(SUM(nettamount),0) AS TotalSale,ROUND(SUM(nettamount),0) AS Profit 
         FROM therapybookingdetails AS a JOIN paitentmaster AS b ON 
   a.paitentid = b.paitentid LEFT JOIN consultationmaster AS c ON a.therapyid = c.consultationid WHERE closingdate > '2022-08-01' 
   AND closingdate BETWEEN '$FromDate' AND '$ToDate'   AND referedbydoctorid LIKE ('$userid')  AND   bookingstatus ='Closed' and 
     bookingid   in ($SelectedBill)   ");
        ...
        FROM therapybookingdetails AS a JOIN paitentmaster AS b ON 
   a.paitentid = b.paitentid LEFT JOIN consultationmaster AS c ON a.therapyid = c.consultationid WHERE closingdate > '2022-08-01' 
   AND closingdate BETWEEN '$FromDate' AND '$ToDate'   AND doctorid LIKE ('$userid')  AND   bookingstatus ='Closed' 
   and bookingid not in (SELECT billuniqueid FROM doctorsharebilldetails where billtype ='Therapy') ");
        } else {
          $result = mysqli_query($connection, "SELECT ROUND(SUM(nettamount),0) AS TotalSale,ROUND(SUM(nettamount),0) AS Profit 
         FROM therapybookingdetails AS a JOIN paitentmaster AS b ON 
   a.paitentid = b.paitentid LEFT JOIN consultationmaster AS c ON a.therapyid = c.consultationid WHERE closingdate > '2022-08-01' 
   AND closingdate BETWEEN '$FromDate' AND '$ToDate'   AND doctorid LIKE ('$userid')  AND   bookingstatus ='Closed' and 
     bookingid  in ($SelectedBill)   ");
        }
      }
    }
