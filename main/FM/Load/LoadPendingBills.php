...
if (isset($_POST["userid"])) {
  ...
  if ($Status == '0') {
    ...
    $Status = '   and consultationuniquebill NOT IN (SELECT billuniqueid FROM doctorsharebilldetails)  ';
    $StatusInventory = '   and saleuniqueno NOT IN (SELECT billuniqueid FROM doctorsharebilldetails)  ';
    $StatusTherapy = "   and bookingid NOT IN (SELECT billuniqueid FROM doctorsharebilldetails where billtype='Therapy')  ";
  } else {
    $Status = '  and  consultationuniquebill  IN (SELECT billuniqueid FROM doctorsharebilldetails)  ';
    $StatusInventory = '   and saleuniqueno NOT IN (SELECT billuniqueid FROM doctorsharebilldetails)  ';
    $StatusTherapy = "  and  bookingid  IN (SELECT billuniqueid FROM doctorsharebilldetails  where billtype='Therapy') ";
  }
  ...
  if ($ShareFor == 'Inventory') {
   ...
   ORDER BY saledate DESC ");
    ...
    if ($DoctorTherapist == 'Doctor') {
     ...
     AND closingdate BETWEEN '$FromDate' AND '$ToDate'   AND referedbydoctorid LIKE ('$userid')  AND   bookingstatus ='Closed' $StatusTherapy  ");
    } else {
      $result = mysqli_query($connection, "SELECT bookingid,DATE_FORMAT(reviseddate,'%d-%m-%y'),DATE_FORMAT(closingdate,'%d-%m-%y'), b.paitentname,c.`consultationname`,
      nettamount  FROM therapybookingdetails AS a JOIN paitentmaster AS b ON 
     a.paitentid = b.paitentid LEFT JOIN consultationmaster AS c ON a.therapyid = c.consultationid WHERE closingdate > '2022-08-01' 
     AND closingdate BETWEEN '$FromDate' AND '$ToDate'   AND doctorid LIKE ('$userid')  AND   bookingstatus ='Closed' $StatusTherapy  ");
    }
   ...
   <td  width='%' >$data[1]</td>  
    ...
    LEFT JOIN doctorsharebilldetails AS d ON a.consultationuniquebill=d.billuniqueid 
