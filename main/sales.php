...
<option></option>
	<?php
	include('../connect.php');
	$result = $db->prepare("SELECT * FROM productmaster AS a JOIN stock AS b ON a.productid = b.`productid` WHERE locationcode='$LocationCode' and currentstock > 0");
...
<input type="text" name="discount" value="" placeholder="Disc %" autocomplete="off" style="width: 68px; height:30px; padding-top:6px; padding-bottom: 4px; margin-right: 4px; font-size:15px;" />
...
<table class="table table-bordered" id="resultTable" data-responsive="table">
	...
	<tbody>
			...
			<?php
				$id=$_GET['invoice'];
				include('../connect.php');
				$result = $db->prepare("SELECT * FROM sales_order WHERE locationcode='$LocationCode' and invoice= :userid");
			...
			<tr>
				<th colspan="7"><strong style="font-size: 12px; color: #222222;">Total:</strong></th>
				...
				$sdsd=$_GET['invoice'];
				$resultas = $db->prepare("SELECT sum(amount) FROM sales_order WHERE locationcode='$LocationCode' and invoice= :a");
			...
			<?php 
				$resulta = $db->prepare("SELECT sum(profit) FROM sales_order WHERE locationcode='$LocationCode' and invoice= :b");
