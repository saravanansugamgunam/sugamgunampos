...
$data = file_get_contents('https://sugamgunam.com/pos/main/EC/wp_order/orders.php');
...
<tr>
    ...
    <td>
        <?= $row['number']; ?></td>
    <td> <?= $row['billing']['first_name']; ?>
        <?= $row['billing']['last_name']; ?> </td>

    <td><?= $row['billing']['phone']; ?></td>
    <td><?= mb_substr($row['date_created'], 0,10 ); ?></td>
    <td><?= $row['status']; ?></td>
    <td><?= $row['total']; ?></td>
    <?php $OrderId = $row['number']; ?>
    <td><?php echo "<a href='OrderView.php?MID=1&orderid= $OrderId'
            target='_blank' ?><i class='fa fa-2x fa-eye' title='View'></i></a>"; ?> </td>
</tr>
<?php $i++; ?>
<?php endforeach; 
